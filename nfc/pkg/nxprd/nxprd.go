...
// First cards to implement opcodes - MIFARE cards
...
phStatus_t MifareUL_Pwd_Auth(void *pwd)
{
	phStatus_t status;
    uint8_t packBuffer[PHAL_AUTH_PACK_LENGTH];

    status = phalMful_PwdAuth(&salMfu, pwd, packBuffer);
	CHECK_SUCCESS(status);
}
*/
import "C"
...
type Writer interface {
	...
	PwdAuth(password string) error
}

// ForumTwoReader implements the Reader interface for an NFC Forum Type 2/ISOIEC14443 Type A compliant
// card.
type ForumTwoReader struct {
}

const (
	// ForumTwoABlockLength defines the block length in bytes,
	// i.e. a block of a Mifare Ultralight contains 4 bytes.
	ForumTwoABlockLength = 4
)

// ReadBlock implements reading a block of bytes for a NFC Forum Type 2/ISOIEC14443 Type A compliant card.
// idx defines the block to be read. See also the Reader interface.
func (r *ForumTwoReader) ReadBlock(idx int) ([]byte, error) {
	libLock.Lock()
	...
	buffer := make([]byte, ForumTwoABlockLength)
	for i := 0; i < ForumTwoABlockLength; i++ {
...
func (r *ForumTwoReader) ReadNdef() (*Ndef, error) {
	buf := new(bytes.Buffer)
	for i := selectedProfile.UserBlockStartIndex; i <= selectedProfile.UserBlockEndIndex; i++ {
...
// ForumTwoWriter implements the Writer interface for an NFC Forum Type 2/ISOIEC14443 Type A compliant
// card.
type ForumTwoWriter struct {
}

// WriteBlock implements writing a block of bytes to a NFC Forum Type 2/ISOIEC14443 Type A compliant card.
// idx defines the memory position / block number.
// See also the Writer interface.
func (w *ForumTwoWriter) WriteBlock(idx int, data []byte) error {
	libLock.Lock()
	defer libLock.Unlock()
	var buffer [ForumTwoABlockLength]byte
	// Only ForumTwoABlockLength bytes will be copied to buffer.
	copy(buffer[:], data)
...
func (w *ForumTwoWriter) WriteNdef(ndef *Ndef) error {
...
func (w *ForumTwoWriter) WriteString(payload string, language Language) error {
	ndef := createDefaultNdefForString(payload, language)
	return w.WriteNdef(&ndef)
}

//https://github.com/svvitale/nxppy/commit/49e081bd9a028dfde371fd7f6283662344fb7269
func (w *ForumTwoWriter) PwdAuth(password string) error {
	libLock.Lock()
	defer libLock.Unlock()
	var buffer [ForumTwoABlockLength]byte
	// Only ForumTwoABlockLength bytes will be copied to buffer.
	copy(buffer[:], password)
	pwd := unsafe.Pointer(&buffer[0])
	// Hand over a void pointer of the array containing the bytes to a C
	// function, which will do the actual authentication.
	status := C.MifareUL_Pwd_Auth(pwd)
	if status != C.PH_ERR_SUCCESS {
		fmt.Println(int(status))
		return createExtErr(AuthErr, fmt.Sprintf("Authentication failed - %d", int(status)))
	}
	return nil
}
...
func writeToMifareUL(ndefBytes []byte, writer *ForumTwoWriter) error {
	...
	data[2] = ndefBytes[0]
	data[3] = ndefBytes[1]
	err := writer.WriteBlock(selectedProfile.UserBlockStartIndex, data)
	if err != nil {
		return err
	}
	...
	for i := selectedProfile.UserBlockStartIndex + 1; i < selectedProfile.UserBlockStartIndex + 1 + exitCondition; i++ {
...
func extractByte(coordinates TagCoordinate) (bool, error) {
	status := C.MifareUL_Read_Block(C.uint8_t(coordinates.ExpectedBlock))
	if status != C.PH_ERR_SUCCESS {
		return false, createLibErr(int(status))
	}
	if byte(C.mfulDataBuffer[coordinates.ExpectedIndex]) == coordinates.ExpectedByte {
		return true, nil
	} else {
		return false, nil
	}
...
func checkTagProfile() (TagProfile, error) {
	standardProfile := MifareULProfile

	nxpProfiles := []TagProfile{
		NTAG203Profile,
		NTAG213Profile,
		NTAG215Profile,
		NTAG216Profile,
	}
	for _, profile := range nxpProfiles {
		nxpTag, err := extractByte(profile.IdentificationByte)
		if err != nil {
			return profile, err
		}
		if nxpTag {
			profileMatch, err := extractByte(profile.TagLengthByte)
			if err != nil {
				return profile, err
			}
			if profileMatch && profile.ProfileName != NTAG203Profile.ProfileName{
				return profile, nil
			}
			// the only apparent differentiation between NTAG203 and 213 is the internal labelled byte in the manufacturers space (block 2, index 1)
			// for 203 it should be 0x00 and for 213 0x48
			// so if the tag is 203 we need to make diff check with 213
			if profileMatch && profile.ProfileName == NTAG203Profile.ProfileName {
				internalByteMatch, err := extractByte(NTAG213Profile.InternalByte)
				if err != nil {
					return profile, errors.Wrap(err, "Could not extract internal byte from NXP tag")
				}
				if internalByteMatch {
					return NTAG213Profile, nil
				}
			}
		}
...
var selectedProfile TagProfile
	...
	for {
		...
		default:
			...
			if res == C.DR_FOUND {
				...
				if C.DetectTypeA() == C.mifare_ultralight {
					// We detected a NFC Forum Type 2/ISOIEC14443 Type A compliant card, so we set
					// Reader, Writer and Params accordingly.
					profile, err := checkTagProfile()
					if err != nil {
						log.WithField("error ", err).Debug("Discover")
					}
					dev.Params.TagProfile = profile
					dev.Params.DevType = profile.ProfileName
					dev.Params.BlockCapacity = profile.TotalBlocks
					dev.Reader = &ForumTwoReader{}
					dev.Writer = &ForumTwoWriter{}
					selectedProfile = profile
				}
				log.WithField("cardType", dev.Params.DevType).Debug("Discover")
				return dev, nil
			}
		}
	}
